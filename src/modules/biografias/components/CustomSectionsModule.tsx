...
export function CustomSectionsModule({ currentBio, role, onUpdateBio }: CustomSectionsModuleProps) {
 ...
 <Sparkles className="w-3.5 h-3.5" style={{ color: brand.colorHex }} /> Galería & Secciones Custom
 </h3>
 
 {isCreator && (
 <button
 onClick={() => setShowAddSection(!showAddSection)}
 className="text-[10px] px-2 py-1 rounded-md font-bold flex items-center gap-0.5 transition-colors" style={{ backgroundColor: `${brand.colorHex}12`, color: brand.colorHex }}
 >
 ...
 <form onSubmit={handleCreateSection} className="p-3 rounded-xl border space-y-2.5" style={{ backgroundColor: `${brand.colorHex}0D`, borderColor: `${brand.colorHex}25` }}>
  <p className="text-[10px] font-bold uppercase tracking-wide" style={{ color: brand.colorHex }}>Nueva Sección Personalizada</p>
 <div>
 ...
 <button
 type="submit"
 className="px-3 py-1 text-white rounded-md font-bold text-[10px] hover:opacity-90" style={{ backgroundColor: brand.colorHex }}
 >
 Crear Sección
 </button>
 </div>
 ...
 {isCreator && <span className="block font-bold mt-1" style={{ color: brand.colorHex }}>¡Sube fotos o guías arriba!</span>}
 </div>
 ) : (
 ...
 <div className="w-12 h-12 rounded-lg overflow-hidden flex-shrink-0 flex items-center justify-center" style={{ backgroundColor: `${brand.colorHex}12`, borderColor: `${brand.colorHex}25` }}>
 {res.type ==='foto' ? (
 <img 
 src={res.url} 
 alt={res.title} 
 ...
 <FileText className="w-5 h-5" style={{ color: brand.colorHex }} />
 ) : (
 ...
 className="p-1 bg-white hover:bg-slate-100 text-slate-500 rounded-md border border-slate-100 shadow-3xs transition" style={{ '--hover-color': brand.colorHex } as React.CSSProperties}
 >
 ...
 onClick={() => setResType(t)}
  ...
  style={resType === t ? { backgroundColor: brand.colorHex } : undefined}
  >
 {t ==='foto' &&' Foto'}
 {t ==='documento' &&' Doc'}
 {t ==='enlace' &&' Link'}
 ...
 <button
 type="submit"
 className="px-2.5 py-1 text-white rounded font-bold text-[9px] hover:opacity-90" style={{ backgroundColor: brand.colorHex }}
 >
 Publicar Recurso
 </button>
 </div>
 ...
 className="w-full py-1.5 text-center text-[10px] font-bold border border-dashed border-slate-200 rounded-xl mt-1.5 transition-colors" style={{ color: brand.colorHex }}
 >
