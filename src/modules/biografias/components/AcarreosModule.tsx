...
import { Truck, MapPin, Phone, DollarSign } from 'lucide-react';
...
export function AcarreosModule({ currentBio }: Props) {
  const data = currentBio.acarreos;
  if (!data) return null;
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">{data.vehicleType}</p>
        </div>
      </div>
      ...
      <div>
        <h4 className="text-xs font-bold text-slate-700 flex items-center gap-1 mb-2"><Truck className="w-3 h-3" /> Servicios</h4>
        <ul className="space-y-1">
          {data.services.map((s, i) => (
            <li key={i} className="text-[11px] text-slate-500 flex items-center gap-1.5">
              <span className="w-1 h-1 rounded-full bg-orange-400 shrink-0" /> {s}
            </li>
          ))}
        </ul>
      </div>
